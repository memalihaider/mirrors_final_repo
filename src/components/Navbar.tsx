...
import React, { useState, useEffect, useRef } from "react";
import { Menu, Moon, Sun, User, LogIn, LogOut, Bell } from "lucide-react";
import { cn } from "@/lib/utils";
import { motion } from "framer-motion";
import { Pacifico } from "next/font/google";
import { useAuth } from "@/contexts/AuthContext";
...
export default function Header({ onToggle }: HeaderProps) {
  ...
  return (
    <motion.header
      ...
      transition={{ duration: 0.6, ease: [0.23, 0.86, 0.39, 0.96] }}
      className={cn(
        "relative z-40 flex items-center justify-between mx-2 mt-3 mr-2 px-3 py-2 rounded-xl shadow-md ml-52",
        scrolled ? "shadow-lg" : ""
      )}
    >
      {/* 🔊 Hidden audio */}

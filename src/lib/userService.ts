...
// ✅ Update user access pages
export const updateUserAccessPages = async (uid: string, accessPages: string[]) => {
  try {
    if (!isFirebaseConfigured() || !db) {
      return { error: 'Firebase is not configured. Please set up your Firebase environment variables.' };
    }
...
// ✅ Update full user profile (email, displayName, role, accessPages)
export const updateUserProfile = async (
  uid: string,
  data: { email?: string; displayName?: string; role?: 'admin' | 'user'; accessPages?: string[] }
) => {
  try {
    if (!isFirebaseConfigured() || !db) {
      return { error: 'Firebase is not configured. Please set up your Firebase environment variables.' };

...
export interface UserRole {
  ...
  displayName?: string;
  role: "admin" | "user";
  accessPages: string[];   // ✅ Added field
  createdAt: Date;
  updatedAt: Date;
}
...
// ---------------- SIGN IN ----------------
export const signInWithEmail = async (email: string, password: string) => {
  if (!auth) return { user: null, error: "Firebase auth is not initialized" };
  try {
...
export const signOutUser = async (): Promise<{ error: string | null }> => {
  try {
    if (!auth) return { error: "Firebase auth is not initialized" };
    await signOut(auth);
    return { error: null };
  } catch (error: unknown) {
    return { error: error instanceof Error ? error.message : "Unknown error" };
...
// ---------------- UPDATE PROFILE ----------------
export const updateUserProfile = async (displayName: string): Promise<{ error: string | null }> => {
  try {
    if (!auth) return { error: "Firebase auth is not initialized" };
    if (auth.currentUser) {
      await updateProfile(auth.currentUser, { displayName });
...
export const getUserRole = async (uid: string): Promise<UserRole | null> => {
  try {
    ...
    if (userDoc.exists()) {
      ...
      return {
        ...data,
        accessPages: data.accessPages || [],   // ✅ Ensure pages load
...
export const setUserRole = async (
  ...
  role: "admin" | "user",
  displayName?: string,
  accessPages: string[] = []     // ✅ Added pages param
): Promise<{ error: string | null }> => {
  try {
    const userData: UserRole = {
      uid,
      ...
      displayName,
      role,
      accessPages,                // ✅ Store pages in Firestore
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    if (!db) throw new Error("Firestore is not initialized");
...
// ---------------- CREATE ADMIN ----------------
export const createAdminUser = async (): Promise<{ success: boolean; error: string | null }> => {
  try {
    if (!auth) return { success: false, error: "Firebase auth is not initialized" };
    console.log("Creating admin user: admin@mirrorbeauty.com");

    const userCredential = await createUserWithEmailAndPassword(auth, "admin@mirrorbeauty.com", "admin123");
    await updateProfile(userCredential.user, { displayName: "Admin User" });

    await setUserRole(userCredential.user.uid, "admin@mirrorbeauty.com", "admin", "Admin User", [
      "dashboard","bookings","branches","offers","customers","users","profile","staff","services","chat","categories"
    ]); // ✅ Admin has all pages
...
export const createNewUser = async (
  ...
  displayName: string,
  role: "admin" | "user",
  accessPages: string[] = []      // ✅ Add pages param
): Promise<{ success: boolean; error: string | null; user?: User }> => {
  try {
    console.log("Creating new user:", email);
    const currentUser = auth?.currentUser;
    ...
    // ✅ Save accessPages properly
    await setUserRole(userCredential.user.uid, email, role, displayName, accessPages);

    await signOut(secondaryAuth);
    try {
      if ("delete" in secondaryApp && typeof secondaryApp.delete === "function") {
        await secondaryApp.delete();
      }
    } catch (error) {
      console.log("Secondary app cleanup not needed or failed:", error);
    ...
    console.log("New user created successfully without affecting current session");
    return { success: true, error: null, user: userCredential.user };
  } catch (error: unknown) {
    console.error("Error creating new user:", error);
    return { success: false, error: error instanceof Error ? error.message : "Unknown error" };
